...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, device: torch.device, epoch: int, max_norm: float = 0):
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if args.fs:
            fs_keep_rate = None
            fs_start_epoch = args.fs_start
            fs_warm_epoch = args.fs_warm
            fs_keep_rate = adjust_keep_rate(epoch, warmup_epochs=fs_start_epoch, total_epochs=fs_start_epoch + fs_warm_epoch, ITERS_PER_EPOCH=1, base_keep_rate=args.fs_keep)
        else:
            fs_keep_rate = 1
        ...
        # model forward
        output = model(img_data, text_data, fs_keep_rate)
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device, epoch: int):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        pred_boxes = model(img_data, text_data, fs_keep_rate)
...
def evaluate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        output = model(img_data, text_data, args.fs_keep)
